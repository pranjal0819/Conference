        ...
        try:
            ...
            if con.owner == request.user or request.user.is_staff:
                return render(request, self.template_name,
                              {'owner': True, 'slug': kwargs['slug'], 'list1': None, 'list2': None})
            else:
                raise PermissionDenied
        except ObjectDoesNotExist:
    ...
    def post(self, request, **kwargs):
        try:
            ...
            if con.owner == request.user or request.user.is_staff:
                ...
                for l in li:
                    ...
                    try:
                        ...
                        message = render_to_string('pc_confirm_mail.html', {
                            ...
                            'slug': kwargs['slug'],
                            'uid': urlsafe_base64_encode(force_bytes(info[2])).decode(),
                            'token': account_activation_token.make_token(info[2]),
                        })
                        email = (mail_subject, message, settings.EMAIL_HOST_USER, [info[2]])
                        email_list.append(email)
                        try:
                            PcMemberRecord.objects.get(pcCon=con, pcEmail=info[2])
                        except ObjectDoesNotExist:
                            PcMemberRecord.objects.create(pcCon=con, pcEmail=info[2], name=name)
                    except IndexError:
                        pass
                ...
                return render(request, self.template_name,
                              {'owner': True, 'slug': kwargs['slug'], 'list1': list1, 'list2': list2})
            else:
                raise PermissionDenied
        except ObjectDoesNotExist:
            messages.error(request, 'Conference Closed or Deleted')
...
def confirm(request, slug, uidb64, token):
    try:
        ...
        user = None
        con = None
    if user is not None and account_activation_token.check_token(user.pcEmail, token):
